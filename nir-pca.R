rm(list=ls())
setwd("~/Google Drive/NIR wood classification/code/")
library(pls)
library(ggplot2)
library(MASS)
source("pca.R")
...
ggsave(file="lignina_test.pdf",plot=pp,scale=2)
...
pp<-pp+geom_smooth(method=lm)+facet_grid(. ~ method)
ggsave(file="lignina_train.pdf",plot=pp,scale=2)